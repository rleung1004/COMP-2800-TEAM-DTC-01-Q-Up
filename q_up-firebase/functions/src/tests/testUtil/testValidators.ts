...
import {isEmpty, isEmail, validateSignUpData, validateLoginData} from '../../util/validators';
...
describe('Validators',() => {
    ...
    describe('validateSignUpData', () => {
        it('should return invalid and error message of Email: Must not be empty', function () {
            ...
            assert.deepEqual(actualData, {errors: {email: 'Must not be empty'}, valid: false});
        });
        it('should return invalid and error message of email: Must be valid email address', () => {
            ...
            const actualData = validateSignUpData(mockData);
            assert.deepEqual(actualData, {errors: {email: "Must be a valid email address"}, valid: false});
        });
        it('should return invalid and error message of password: Must not be empty', () => {
            ...
            const actualData = validateSignUpData(mockData);
            assert.deepEqual(actualData, {errors: {password: "Must not be empty"}, valid: false});
        });
        it('should return invalid and error message of confirmPassword: Passwords must match', () => {
            ...
            const actualData = validateSignUpData(mockData);
            assert.deepEqual(actualData, {errors: {confirmPassword: "Passwords must match"}, valid: false});
        });
        it('should return valid with empty error if email password and confirm password are valid', () => {
            const mockData = {
                email: 'ali@gmail.com',
                password: '123123',
                confirmPassword: '123123'
